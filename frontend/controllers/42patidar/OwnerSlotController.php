 ...
 * Slot controller
 */
class OwnerSlotController extends Controller
{
    public $_user = array();
    public $_baseUrl = "/";
    public $_module = 'nursing';
    public $_moduleID = '5';
    public $_moduleType = "owner";
    public $_moduleTypePath = '/nursing/owner-slot/';
    public $_moduleTypeUrl = "/";
    public $_moduleTypePathOwner = '/nursing/owner/';
    public $_moduleTypeUrlOwner = '/';
    public $_lang = "en";
    public $_langID = 1;
    public $_profiledata = "";
    ...
    public function beforeAction($action) {
        ...
        if(isset($cookiedestory['result']) && isset($cookiedestory['type']) && $cookiedestory['result']) {
            ...
            Yii::$app->user->logout();
            return $this->redirect([$this->_moduleTypePathOwner . 'login']);
        }
    ...
    public function init() {
        ...
        $this->_moduleTypePath = '/'.$this->_module.'/owner-slot/';
        $this->_moduleTypeUrl = Yii::$app->params[$this->_module.'Url'] . 'owner-slot/';
        $this->_moduleTypePathOwner = '/'.$this->_module.'/'.$this->_moduleType.'/';
        $this->_moduleTypeUrlOwner = Yii::$app->params[$this->_module.'Url'] . ''.$this->_moduleType.'/';
    }
    public function actionCreate()
    {
        ...
        if(Yii::$app->request->post()) {
            ...
            $postData[$prefix]['last_modify_from'] = 'Owner-Web';
            if(isset($postData[$prefix]['invite_my_fav'])){
                $postData[$prefix]['invite_my_fav'] = $postData[$prefix]['invite_my_fav'];
            }else{
                $postData[$prefix]['invite_my_fav'] = 'No';
            }
            if(isset($postData[$prefix]['travel'])){
                $postData[$prefix]['travel'] = $postData[$prefix]['travel'];
            }else{
                $postData[$prefix]['travel'] = 'Fees';
            }
            if(isset($postData[$prefix]['accommodation'])){
                $postData[$prefix]['accommodation'] = $postData[$prefix]['accommodation'];
            }else{
                $postData[$prefix]['accommodation'] = 'Fees';
            }
            
            $removeKeyArr = array('selectDateSingle','single_date', 'single_date_starttime', 'single_date_endtime', 'hour_value_rate', 'hour_value_hour', 'hour_total','selectDateMultidate_start','selectDateMultidate_end', 'multiple_date', 'multiple_date_starttime', 'multiple_date_endtime');
            ...
            if($postData[$prefix][$date_name]) {
                foreach ($postData[$prefix][$date_name] as $key => $value) {
                    $dateArray[$key]['start_datetime'] = $value.' '.$postData[$prefix][$start_time_name][$key].':00';
                    $dateArray[$key]['end_datetime'] = $value.' '.$postData[$prefix][$end_time_name][$key].':00';
                    $dateArray[$key]['availability_id'] = '';
                    $dateArray[$key]['staff_user_id'] = '';
                    $dateArray[$key]['request_valid'] = '';
                }
            }

            $postData['Slot']['schedule'] = json_encode($dateArray);
            $province_id = explode("-",$postData['Slot']['province_id']);
            if($province_id){
                $postData['Slot']['owner_user_id'] = $province_id[0];
                $postData['Slot']['province_id'] = $province_id[1];
            }else{
                $postData['Slot']['owner_user_id'] = "";
                $postData['Slot']['province_id'] = "";
            }
            ...
            $output = Yii::$app->ApiCallFunctions->ownerSlotCreate($restapiData);
            $message = $output['message'];
            if ($output['status'] == 200) {
                Yii::$app->session->setFlash('success', $message);
                return $this->redirect([$this->_moduleTypePathOwner.'dashboard']);
            }else{
        ...
        return $this->render('create', [
            ...
            "moduleTypePathOwner" => $this->_moduleTypePathOwner,
            "moduleTypeUrlOwner" => $this->_moduleTypeUrlOwner,
            "lang" => $this->_lang,
            'user' => $this->_user,
    ...
    public function actionUpdate($slot_id = "")
    {
        ...
        if(empty($slot_id)){
            Yii::$app->session->setFlash('error', Yii::t('app', 'Slot ID not available'));
            return $this->redirect([$this->_moduleTypePathOwner.'dashboard']);
            exit;
        }
        $slot_id = Yii::$app->MyFunctions->decode($slot_id);
        ...
        $outputslot = Yii::$app->ApiCallFunctions->getSingleSlotsDetails($restapiData);
        $slotdata = array();
        $startdate = "";
        $enddate = "";
        if ($outputcat['status'] == 200) {
            $slotdata = $outputslot['data'];
            if($slotdata['start_date'] && $slotdata['end_date']){
                $startdate = date(Yii::$app->params['phpdateFormat'],strtotime(str_replace('/', '-', $slotdata['start_date'])));
                $enddate = date(Yii::$app->params['phpdateFormat'],strtotime(str_replace('/', '-', $slotdata['end_date'])));
            }else{
                $startdate = date(Yii::$app->params['phpdateFormat']);
                $enddate = date(Yii::$app->params['phpdateFormat']);
            }
        }
        
        $model = Slot::find()->where(['slot_id'=>$slot_id])->one();
        $model->scenario = 'slotupdate';
        if(Yii::$app->request->post()) {
            $postData = Yii::$app->request->post();
            $prefix = 'Slot';
            $slot_type = ($postData[$prefix]['type'] == '2') ? 'Multidays' : 'Singleday';
            ...
            $restapiData['slot_id'] = $slot_id;
            //echo "<pre>";print_r($restapiData);exit;
            $output = Yii::$app->ApiCallFunctions->ownerSlotUpdate($restapiData);
            $message = $output['message'];
            if ($output['status'] == 200) {
                Yii::$app->session->setFlash('success', $message);
                return $this->redirect([$this->_moduleTypePathOwner.'slots?date='.$startdate]);
            }else{
                Yii::$app->session->setFlash('error', $message);
            }
        }
                ...
                if($taxdata){
                    ...
                    if($taxdata_json){
                        ...
                        if($Slot_sub_total_price){
                            $Slot_tax_gst_total = (($Slot_sub_total_price*$taxdata_json['owner_gst_precentage'])/100);
                            $Slot_tax_hst_total = (($Slot_sub_total_price*$taxdata_json['owner_hst_precentage'])/100);
                            $Slot_tax_pst_total = (($Slot_sub_total_price*$taxdata_json['owner_pst_precentage'])/100);
                            $Slot_tax_qst_total = (($Slot_sub_total_price*$taxdata_json['owner_qst_precentage'])/100);
                         }else{
                        ...
                        //Item
                        $Slot_tax_gst = (($totalsum_modifiedprice*$taxdata_json['owner_gst_precentage'])/100);
                        $Slot_tax_hst = (($totalsum_modifiedprice*$taxdata_json['owner_hst_precentage'])/100);
                        $Slot_tax_pst = (($totalsum_modifiedprice*$taxdata_json['owner_pst_precentage'])/100);
                        $Slot_tax_qst = (($totalsum_modifiedprice*$taxdata_json['owner_qst_precentage'])/100);
    ...
    public function actionRequestcancellation() {
        if (Yii::$app->user->isGuest) {
            return $this->redirect([$this->_moduleTypePath . 'login']);
        }
        $user = $this->_user;
        $slot_id = "";
        ...
        if (empty($slot_id) || (isset($userModulesetting['SlotCancellation']) && $userModulesetting['SlotCancellation'] == "No")) {
            ...
            return $this->redirect([$this->_moduleTypePath . 'dashboard']);
        }
    ...
    public function actionRequestmodification(){
        ...
        if ($outputmodification['status'] == 200) {
            ...
            if(isset($slotmodificationsData['slot_id']) && $slotmodificationsData['slot_id']){
                ...
                if($slotmodifications && $slotmodifications['change_request_status'] == 2){
                    ...
                    return $this->redirect([$this->_moduleTypePathOwner . 'slotdetails', 'slot_id' => Yii::$app->MyFunctions->encode($slot_id)]);
                }
            }else{
                $slotmodifications = "";
            } 
        ...
        $SlotItemsbackend = SlotItems::find()->where(['slot_id'=>$slot_id,'add_in' => 'Owner'])->all();
        ...
        if ($model->load(Yii::$app->request->post()) && $slot_id) {
            ...
            $restapiData['last_modify_from'] = 'Owner-Web';
