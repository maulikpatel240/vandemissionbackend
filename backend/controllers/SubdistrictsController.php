...
use backend\models\SubdistrictsSearch;
...
/**
 * SubdistrictsController implements the CRUD actions for Subdistricts model.
 */
    ...
    public function actionCreate() {
        ...
        if (Yii::$app->request->isAjax) {
            ...
            if ($model->load(Yii::$app->request->post()) && $model->save()) {
                $model->map = UploadedFile::getInstance($model, 'map');
                if($model->map){
                    $model->deleteImage($model->getOldAttribute('map'));
                    $model->upload();
                }
                $model->status_at = Yii::$app->BackFunctions->currentDateTime();
        ...
        if (Yii::$app->request->isAjax) {
            ...
            if ($model->load(Yii::$app->request->post())) {
                ...
                if($model->map){
                    ...
                    $model->map = $model->getOldAttribute('map');
                }
                $model->updated_at = Yii::$app->BackFunctions->currentDateTime();
                $model->lang_key = str_replace(" ","_",strtolower($model->english)).'_'.$model->id;
                $model->save();
