...
class Subdistricts extends \yii\db\ActiveRecord
{
    ...
    public function upload()
    {
        if($this->map){
            ...
            $filename = 'subdistrict_'.$this->id.'.' . $this->map->extension;
            $this->map->saveAs(Yii::getAlias('@webroot').'/uploads/map/subdistrict/' . $filename, false);
            $this->map = $filename;
            $this->save();
            return true;
        }else{
    ...
    public function deleteImage($oldimg = "") {
        if($oldimg){
            $image = Yii::getAlias('@webroot').'/uploads/map/subdistrict/' . $oldimg;
            if (file_exists($image) && unlink($image)) {
                return true;
            }
        }else{
            $image = Yii::getAlias('@webroot').'/uploads/map/subdistrict/' . $this->map;
            if (file_exists($image) && unlink($image)) {
                $this->map = '';
                $this->save();
                return true;
