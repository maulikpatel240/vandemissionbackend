...
if (!empty(Yii::$app->BackFunctions->checkaccess('create', Yii::$app->controller->id))) {
    $createBtn = Html::a('<i class="fas fa-plus"></i>', FALSE, ['value' => Url::to(['users/create']), 'title' => 'Create country', 'class' => 'showModalButton btn btn-primary']);
}
//Export
$exportBtn = '';
...
//                                    ],
                        [
                            ...
                            'value' => function ($model) {
                                $imagesrc = Html::img(Yii::$app->urlManager->baseUrl . '/uploads/user/avatar/' . $model->avatar, ['width' => '70', 'height' => '70', 'class' => 'img-circle']);
                                ...
                                if (!empty(Yii::$app->BackFunctions->checkaccess('status', Yii::$app->controller->id))) {
                                    return Html::a($model->status,
                                                    ...
                                                    [
                                                        ...
                                                        'title' => $model->status,
                                                        'onclick' => '$.post({
                                                            url: "' . Yii::$app->homeUrl . 'users/statusupdate?id=' . $model->id . '",
                                ...
                                'update' => function ($url, $model) {
                                    if (!empty(Yii::$app->BackFunctions->checkaccess('update', Yii::$app->controller->id))) {
                                        return Html::a(
                                                        ...
                                                        FALSE,
                                                        [
                                                            'value' => Url::to(['users/update', 'id' => $model->id]),
                                                            'title' => 'Edit User',
...
<script type="text/javascript">
    function applyjs(e) {
        ...
        $.post({
            url: '<?= Yii::$app->homeUrl . 'users/applystatus' ?>',
            dataType: 'json',
